// partner-portal/frontend/src/app/dashboard/payouts/page.tsx
'use client';
...
import { PartnerDashboardLayout } from '@/components/partner/PartnerDashboardLayout'; // Placeholder
...
const uploadPayoutBatchFn = httpsCallable(functions, 'uploadPayoutBatch');
const getPayoutLogsFn = httpsCallable(functions, 'getPayoutLogs');
const generateUploadUrlFn = httpsCallable(functions, 'generateUploadUrl'); // New: to get signed URL
...
const PayoutUploader = () => {
    ...
    const mutation = useMutation({
        // The mutation now takes the GCS file path and original payouts data
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        // 1. Parse file content (still needed for sending payouts data to CF)
        let payouts: any[] = [];
        try {
            const text = await file.text();
            payouts = JSON.parse(text); // Assuming JSON format for simplicity
        ...
        // 2. Get signed URL from Cloud Function
        try {
            const generateUrlResult: any = await generateUploadUrlFn({
                fileName: file.name,
                fileType: file.type,
                uploadPath: 'payout-batches' // Specific path within partner's GCS folder
            });
            const { signedUrl, fileMetadata } = generateUrlResult.data;

            // 3. Upload file directly to GCS using the signed URL
            ...
            // 4. Call backend mutation with GCS path and payout data
            mutation.mutate({
                fileName: file.name,
                gcsFilePath: fileMetadata.fullPath, // Send the GCS path to the backend
                payouts: payouts // Still send the parsed data for validation/processing
            });
        ...
        <Card className="bg-gray-800 border border-gray-700 text-white shadow-lg">
            ...
            <CardContent>
                <div {...getRootProps()} className={`p-10 border-2 rounded-lg text-center cursor-pointer transition-colors duration-200
                    ...
                    {mutation.isLoading && uploadProgress > 0 && uploadProgress < 100 && (
