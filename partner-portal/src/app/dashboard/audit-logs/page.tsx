// partner-portal/src/app/dashboard/audit-logs/page.tsx
'use client';
...
export default function PartnerAuditLogsPage() {
    ...
    if (error) return <div className="text-red-500">Error loading audit logs: {(error as Error).message}</div>;
                                ...
                                {logs?.length === 0 ? (
                                    ...
                                    logs?.map(log => (
                                        <tr key={log.id} className="hover:bg-gray-700 transition-colors duration-150">
                                            ...
                                            <td className="px-4 py-3 text-sm font-mono text-gray-400 break-all">{log.details ? JSON.stringify(log.details) : 'N/A'}</td>
                                        </tr>
                                    ))
                                )}
                            </tbody>
