...
// HTTP-callable function to check if the user's password has expired
export const checkPasswordExpiration = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const passwordLastChanged = userData?.profile?.passwordLastChangedAt as admin.firestore.Timestamp | undefined;
        
        // If timestamp doesn't exist, treat as expired to encourage setting it
        const hasExpired = passwordLastChanged ? isMoreThan90DaysAgo(passwordLastChanged) : true;
...
// Callable function to handle password changes
...
export const changePassword = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Step 2: Update the passwordLastChangedAt timestamp in Firestore
        ...
        throw new functions.https.HttpsError('internal', 'Failed to change password.', error.message);
    }
});
