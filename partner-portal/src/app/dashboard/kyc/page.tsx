// partner-portal/src/app/dashboard/kyc/page.tsx
'use client';
...
const submitKycFn = httpsCallable(functions, 'partner-submitKycForMerchant');
const getKycSubmissionsFn = httpsCallable(functions, 'partner-getKycSubmissions');
...
interface SecureFileUploadProps {
    label: string;
    onFileUploaded: (gcsUrl: string) => void;
    uploadPath: string;
...
const SecureFileUpload = ({ label, onFileUploaded, uploadPath, acceptedFileTypes, isLoadingParent }: SecureFileUploadProps) => {
    ...
    const uploadMutation = useMutation({
        mutationFn: async (file: File) => {
            setUploadProgress(0);
            setUploadedUrl(null);
            const { data: { signedUrl, fileMetadata } } = await (generateUploadUrlFn({
                fileName: file.name,
                fileType: file.type,
                uploadPath: `kyc-documents/${uploadPath}`
            }) as Promise<any>);
        ...
        onSuccess: (gcsUrl) => {
            ...
            onFileUploaded(gcsUrl);
            toast({ title: "Upload Complete", description: `${label} uploaded successfully.`});
        },
        onError: (error: any) => {
    ...
    const handleFileChange = useCallback((event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file) {
            if (uploadMutation.isPending || isLoadingParent) return;
            setCurrentFile(file);
...
const KycSubmissionForm = () => {
    ...
    const mutation = useMutation({
        ...
        onSuccess: () => {
            ...
            methods.reset();
            setBusinessRegistrationUrl(null);
    ...
    const isSubmitDisabled = mutation.isPending || !businessRegistrationUrl || !signatoryIdFrontUrl || !signatoryIdBackUrl || !methods.formState.isValid;
                ...
                <form onSubmit={methods.handleSubmit((data) => mutation.mutate(data))} className="space-y-4">
                    ...
                    <div className="space-y-4 pt-2">
                        ...
                        <SecureFileUpload 
                            ...
                            isLoadingParent={mutation.isPending}
                        />
                         <SecureFileUpload 
    ...
    return (
         <Card className="mt-6 bg-gray-800 border border-gray-700 text-white shadow-lg">
            ...
            <CardContent>
                    ...
                    <table className="min-w-full divide-y divide-gray-700">
                        ...
                        <tbody className="divide-y divide-gray-700">
                            {submissions?.length === 0 ? (
                                <tr>
                                    <td colSpan={3} className="text-center py-4 text-gray-400">No KYC submissions found.</td>
                                </tr>
                            ) : (
                                submissions?.map(sub => (
             ...
             {submissions?.some(sub => sub.rejectionReason) && (
